...
const E = setup({
  timeoutFlat: 1,
  tmpName: "_tmp_curvetest.js",
  transform: code => (RR ? code.replace(/const rr=\(\)=>[^;]+;/, `const rr=()=>(1-${RR}/2)+Math.random()*${RR};`) : code),
  exports: `makeCard,FORMS,buildTeam:(c,s)=>buildTeam(c,s),
    _startWith:(h,a)=>{MC={home:h,away:a,min:0,ball:50,bx:50,by:50,idx:0,name:'T',lv:0,subs:3,halt:false,loop:false};return MC;},
    runLoop:()=>runLoop()`,
});
(async () => {
  await E.boot();
  ...
  for (const f of [0.80, 0.90, 0.95, 1.00, 1.05, 1.10, 1.20]) {
    ...
    for (let g = 0; g < N; g++) {
      E._startWith(clone("H", 1), clone("A", f)); const M = E.getMC(); await E.runLoop();
