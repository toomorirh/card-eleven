...
// 現在の育成編成の素OVR合計(先発XIのみ・上限判定/表示用・成長は非加算)。ベンチは統制対象外。
...
function careerBaseTotal(cr){
  ...
  return team.players.reduce((s,p)=>s+cardOvr(p.c),0); // 先発XIのみ(ベンチは統制対象外)
}
// 統制ペナルティ倍率: 編成OVR(素・先発XIのみ)が統制可能OVR(careerCap)を超えた超過率ぶん全能力を低下。
...
function homeBaseTotal(home){
  return (home&&home.players?home.players:[]).reduce((s,p)=>s+cardOvr(p.c),0);
}
...
function worldTeam(nation,idx){
  ...
  // シグネチャー注入(共通ルール): 同じ細分sub優先 → 同じメインポジ(大分類FW/MF/DF/GK)内の空き枠。
  // どちらも空きが無ければ注入しない(GKや他ラインへは移さない=素のカードのまま)。配置枠のsubRoleを継承。
  if(typeof SIGNATURES!=="undefined"&&typeof makeSignature==="function"){
    SIGNATURES.filter(s=>s.flag===nation.flag).forEach(sig=>{
      ...
      if(i<0)i=cards.findIndex(p=>!p._sig&&subGroup(p.subRole)===sig.pos); // 同メインポジ内の空きのみ
      if(i>=0){const sc=makeSignature(sig.id);
        cards[i]={c:sc,role:subGroup(cards[i].subRole),subRole:cards[i].subRole,pen:1,x:cards[i].x,y:cards[i].y,enter:0,
...
// 攻撃采配の起点判定: 持ち主のsubRoleが tac.from の起点ポジ群に含まれるか(守備采配は含まれず攻撃時は発火しない)。
function tacFromMatch(tac,carrier){return tacFromSubs(tac.from).includes(carrier&&carrier.subRole);}
