// ================= 状態と保存 =================
let S={coins:300,coll:[],squad:{},form:"4-4-2",cleared:0,tactic:"bal",v:9,legendPacks:0,championPacks:0,sigPacks:0,sigSelect:0,ms:{},league:null};
...
function applyDefaults(){
  S={coins:300,coll:[],squad:{},form:"4-4-2",cleared:0,tactic:"bal",v:9,legendPacks:0,championPacks:0,sigPacks:0,sigSelect:0,ms:{},league:null};
...
async function loadGame(){                                       // つづきから(セーブが無ければ新規)
  ...
  if(checkMilestones())await save();  // 旧セーブが既に条件を満たしていれば付与
}
...
// 固有選手の入手: 未達成のマイルストーンを判定し、満たしていればチケットを付与(達成済みはS.msで記録)。
function checkMilestones(){
  S.ms=S.ms||{};let got=false;
  for(const m of MILESTONES){
    if(!S.ms[m.id]&&m.test()){
      S.ms[m.id]=1;
      if(m.kind==="select")S.sigSelect=(S.sigSelect||0)+1; else S.sigPacks=(S.sigPacks||0)+1;
      toast(m.msg);got=true;
    }
  }
  return got;
}
