...
function startCareerMatch(){ // ①リーグ / カップ戦の1試合。上限内編成で相応の相手と対戦。
  ...
  const label=cr.cup?`${cr.cup.emoji} ${cr.cup.name} 第${cr.cup.i+1}/${cr.cup.need}戦: ${cr.oppName}`:`DIV${cr.div} 第${cr.node+1}節: ${cr.oppName}`;
...
function finalizeCareerIfDone(){ // 48ステップ到達→カスタム監督を確定して登録
  ...
  if(cr.cup)return false; // カップ進行中は任期を延長(決着まで確定しない)
