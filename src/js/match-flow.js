  ...
  career:{ async onEnd(M,sh,sa){ // 監督キャリアのリーグ/カップ1試合
    ...
    if(inCup){
      ...
      if(o.champion){champCup=o.cup; pp+=15;
        const rw=((cupById(o.cup.id)||{}).reward)||o.cup.reward||{}; // reward はCUPS正本から引く(cr.cupにはrewardを持たせていないため)
        if(rw.coins)S.coins=(S.coins||0)+rw.coins; if(rw.sigPacks)S.sigPacks=(S.sigPacks||0)+rw.sigPacks; // 賞金+シグネチャーパック
        if(typeof coinUI==="function")coinUI(); // 付与したコインをヘッダへ即反映
