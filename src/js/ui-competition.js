...
function exportTeam(){
  ...
  const w=_BW();
  w.push(Math.max(0,_FORMS.indexOf(S.form)),4); // フォメindexは4bit(最大16種・旧3bitのQRとは非互換)
  ...
  const bits=w.done();
  const head=[0xC6,coach.length,...coach,team.length,...team]; // 0xC6=v6(フォメ4bit)。0xC5/0xC4/0xC3/0xC2は読込互換
...
function importTeam(raw){
  ...
  const ver=bytes[0]; if(ver!==0xC2&&ver!==0xC3&&ver!==0xC4&&ver!==0xC5&&ver!==0xC6)throw new Error("bad");
  const sigBits=(ver===0xC3||ver===0xC4||ver===0xC5||ver===0xC6)?10:5; // sig ビット幅(v2=5/v3以降=10)
  const typeBits=(ver===0xC4||ver===0xC5||ver===0xC6)?3:2;             // type ビット幅(v4以降=3で最大8種/ポジション)
  const ageBits=(ver===0xC5||ver===0xC6)?5:0;                          // age ビット幅(v5以降=5。以前は非同梱→AGE_DEF)
  const formBits=(ver===0xC6)?4:3;                                     // フォメ index 幅(v6以降=4bitで最大16種・旧版は3bit)
  ...
  const form=_FORMS[r.read(formBits)]||"4-4-2", favFlag=r.read(1), kp=KEYPOS[form]||{};
...
function renderManagers(){
  ...
  owned.forEach(m=>{
    ...
    const info=mk("div","wt-info");info.innerHTML=`<div class="wt-name">${m.title}${isAct?' <span class="lv" style="color:var(--gold)">起用中</span>':''}</div><div class="lv">${m.name} ・ 🔼 ${mgrBoostDesc(m)}${mgrTacDesc(m)?` ・ ${mgrTacDesc(m)}`:""}</div><div class="lv">🧭 統制OVR <b style="color:var(--gold)">${mgrCtrlOVR(m)}</b></div>`;
    d.appendChild(info);
...
function careerCurrentActivity(cr){
  ...
  // 任期満了パネルは finalizeCareerIfDone が実際に確定/延長オファーへ進める時のみ表示する。
  // カップ/大陸シーズン進行中(cup/contId)は step が stepsMax を超えていても確定を保留=決着まで継続プレー
  // (パネルを出すと「①任期を締める」が finalizeCareerIfDone=false で無反応になるため、下の通常操作へフォールスルー)。
  if(cr.step>=(cr.stepsMax||CAREER.steps)&&!cr.cup&&!cr.contId){wrap.innerHTML=`<div class="cn-head">🎓 任期満了</div><div class="lg">「①」で監督を確定します</div>`;
...
function renderCareer(){
  ...
  // ハブ(ステータス + 今週の活動): 進行系タブ(日程/リーグ/カップ)のみ表示。
  // 編成/施設/監督は進行に関わらないため、上部サマリー・操作枠は出さない。
  if(_careerTab==="schedule"||_careerTab==="league"||_careerTab==="cup"){
