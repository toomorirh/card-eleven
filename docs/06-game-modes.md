...
- **キャリアループ(フェーズ2・実装済み)**: `CAREER`設定。任期 `steps`=48・1ステップ=1試合。`S.career{name,step,div,node,pts,gf,ga,ovrCap,boosts[],tacs[]}`。
  ...
  - **名将クラブ(壁・実装済み)**: `OPP_CLUBS` の `legend:true` 群=**各名将(16名)が率いる平均OVR≈105(lv11)の超強豪16クラブ**(`mgr`=名将id・`legendClubIds()`)。lv11は `poolLv`(5-10)外なので通常プールには出ず、**明示注入のみ**。**国際クラブカップ**は `drawCupBracket` がブラケットに**1-2チーム混入**、**大陸リーグ**は `startCont` が6節のうち**1-2節を差し替え**(`cr.contBosses`=`[{node,id}]`・シーズン終了でクリア)。`oppTeam` は `club.mgr` 指定でその**特定名将を起用**(Tier別ランダムを上書き・`t.legend`)。優勝を阻む壁として登場。表示は **💫名将** バッジ(順位表/ブラケット/偵察・`oppBadge`)。
  - **撃破報酬(実績+称号)**: 名将クラブに勝利すると `S.legendBeaten[id]=1` を記録→`checkAchievements` が該当実績(`beat_<id>`・`ACHIEVEMENTS` に `legendClubIds` から自動生成)を解除し **シグネチャーパック+称号(クラブ名)** を付与(`grantReward` の `title` で `S.titles` へ)。称号システムは §2.4/プロフィールで装備。※後日「名将クラブ攻略イベント」の拡張余地(`legend` 目印)。
  - **相手クラブの見える化(名前付き・Tier・偵察)**: `OPP_CLUBS`(名前/lv/form/**seedでロスター固定**=偵察の意味が出る・👑=看板ボス・💫=名将クラブ)。
