...
- **戦術**: `oppAdjustMin`(60分)にビハインドなら`atk`/リードなら`def`へ切替(既存)。攻撃スタイルは `oppPickStyle`。
- **まだ非対称な点**: 監督ブースト/采配は自チーム限定(相手はボス級の小さな `aiMgr` 全能力ブーストのみ)。→ 相手監督のTier別バフ・采配・性格(戦術/交代判断)は今後実装予定(§7.10 末)。
...
- **データ**(`data.js` `MANAGERS`×**16**): `col/row`(画像シート内グリッド)・`sheet`(1 or 2=使用ポートレートシート)・`boost{pos,stat,mul}`(自チームのみ対象ポジ×ステを乗算)・`cost`(起用ごとのレンタル料)・`tac`(采配シグネ)。実在監督のカリカチュア画像に対応。
  - **シート1**(`src/assets/manager/ce_mg_managers.png`・4×2): グアルディオラ(MF技/電光タクト)・クロップ(全速/アーリークロス)・アンチェロッティ(全ステ薄)・モウリーニョ(DF守/電撃カウンター)・ファーガソン(全スタミナ)・ジダン(FW攻)・ヴェンゲル(全技)・シメオネ(全守/密集ブロック)。
  - **シート2**(`src/assets/manager/ce_mg2_managers.png`・`sheet:2`): クライフ(MF攻/フライング・ダッチマン)・サッキ(MF速/オフサイドトラップ)・ビエルサ(全スタ/縦への衝動)・コンテ(全力/ウイングバック強襲)・ジーコ(FW技/黄金のパスワーク)・リトバルスキー(MF技)・デル・ボスケ(全ステ薄)・カペッロ(DF守)。
  - ブーストは **+3〜8%程度の小幅**(`mul` 1.03〜1.08)。`mgrPortrait(m,h)` が `sheet` に応じ `MGR_SHEET`/`MGR_SHEET2` から全身を切り出す。
