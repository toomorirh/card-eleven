...
- **データ**(`data.js` `MANAGERS`×8): `col/row`(画像シート内グリッド)・`boost{pos,stat,mul}`(自チームのみ対象ポジ×ステを乗算)・`cost`(起用ごとのレンタル料)・`tac`(采配シグネ)。**8名は実在監督のカリカチュア画像**(`src/assets/manager/ce_mg_managers.png`・4×2)に対応し、見た目から名前/個性を設定: グアルディオラ(MF技/電光タクト)・クロップ(全速/アーリークロス)・アンチェロッティ(全ステ薄)・モウリーニョ(DF守/電撃カウンター)・ファーガソン(全スタミナ)・ジダン(FW攻)・ヴェンゲル(全技)・シメオネ(全守/密集ブロック)。
